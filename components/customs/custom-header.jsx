...
import { StyledHeader, StyledBox, StyledTitle } from '@/style/header.style'

const CustomHomeHeader = ({ title = '(stack) header', background = 'container', iconLeft, iconLeftParams, iconRight, iconRightParams }) => {
  const { top } = useSafeAreaInsets()
  const { navigate, goBack, dispatch } = useNavigation()

  return (
    <View background={background} style={{ paddingTop: top, position: 'absolute', width: '100%' }}>
      <StyledHeader>
