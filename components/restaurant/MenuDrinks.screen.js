...
import { VerticalList, VerticalListItem, VerticalListLine } from 'styles/list.module'
...
export default function MenuDrinksScreen({ navigation, route }) {
  ...
  const { data, refetch } = useQuery(['@restaurant-drink-menu', _id], () => restaurantDrinkCateogry(_id), {
    refetchOnMount: true,
    initialData: _data,
  })
  ...
  return (
    <AreaView refreshControl={<RefreshControl refreshing={refresh} onRefresh={onRefresh} />}>
      <Text size={16} content={'Order your drinks from here and enjoy a refreshing beverage with your meal.'} />
      <VerticalList>
        <VerticalListItem onPress={() => setCategory('SOFT')}>
          <Text content={'soft drinks'} weight={700} size={18} up={'up'} color={isCategory === 'SOFT' ? 'dominant' : 'gray'} />
        </VerticalListItem>
        <VerticalListLine />
        <VerticalListItem onPress={() => setCategory('ALCOHOL')}>
          <Text content={'alcohol drinks'} weight={700} size={18} up={'up'} color={isCategory === 'ALCOHOL' ? 'dominant' : 'gray'} />
        </VerticalListItem>
      </VerticalList>
      <GridLayout>
        {data
          ...
          .map((el, key) => {
            return (
              <SimpleCard
                ...
                onPress={() =>
                  navigation.navigate(
                    el.restaurant_drink_type === 'ALCOHOL' ? 'menu-tab-stack-restaurant-detail-menu-drinks-details-alcohol' : 'menu-tab-stack-restaurant-detail-menu-drinks-details',
                    {
                      _id: el.id,
                      _data: el.restaurant_drink_type === 'ALCOHOL' ? el.alcoholdrinks : el.softdrinks,
...
let restaurantDrinkCateogry = function (id) {
  return __query
    .get(`api/restaurant/drink?query=${id}`)
    .then((res) => res.data)
    .catch((err) => {
